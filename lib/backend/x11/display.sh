...
function zenbook-enable-bottom-monitor() {
    ...
    xrandr \
        --output "${TOP_OUTPUT}" --primary --mode "${TOP_MODE}" --rate "${TOP_RATE}" --rotate normal --pos 0x0 \
        --output "${BOTTOM_OUTPUT}" --mode "${BOTTOM_MODE}" --rate "${BOTTOM_RATE}" --rotate normal --below "${TOP_OUTPUT}"
    local rc=$?
...
function zenbook-mirror-displays() {
    ...
    xrandr \
        --output "${TOP_OUTPUT}" --primary --mode "${TOP_MODE}" --rate "${TOP_RATE}" --rotate normal --pos 0x0 \
        --output "${BOTTOM_OUTPUT}" --mode "${BOTTOM_MODE}" --rate "${BOTTOM_RATE}" --rotate normal --same-as "${TOP_OUTPUT}"
    rc=$?
